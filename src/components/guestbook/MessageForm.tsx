...
export default function MessageForm({ onSubmit, onCancel }: MessageFormProps) {
  ...
  return (
    <form
      ...
      className="p-6 rounded-2xl mb-10 space-y-4"
      style={{
    ...
    >
      <h3 className="font-serif text-xl font-bold text-white">写留言</h3>
      ...
      <div className="flex gap-3">
        <button
          type="submit"
          disabled={submitting}
          className="flex-1 py-3 rounded-xl text-sm font-medium text-white transition-all duration-300 disabled:opacity-50"
          style={{
        ...
        {onCancel && (
          <button
            ...
            onClick={onCancel}
            disabled={submitting}
            className="px-5 py-3 rounded-xl text-sm font-medium text-white/50 border border-white/[0.08] bg-white/[0.04] hover:text-white/80 hover:border-white/20 transition-colors disabled:opacity-50"
          >
            取消
          </button>
        )}
